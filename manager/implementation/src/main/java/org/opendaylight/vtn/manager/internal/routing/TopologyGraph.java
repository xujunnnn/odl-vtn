...
final class TopologyGraph extends SparseMultigraph<SalNode, LinkEdge> {
    ...
    private final Map<ImmutablePair<SalNode, SalNode>, List<List<LinkEdge>>> paths = new ConcurrentHashMap<>();
    private final Map<ImmutablePair<SalNode, SalNode>, Integer> pathindex = new ConcurrentHashMap<>();
    ...
    private class DijkstraResolver
		...
		@Override
		public List<LinkEdge> gettcpRoute(InventoryReader rdr, SalNode src, SalNode dst) {
			// TODO Auto-generated method stub
			 if (src.getNodeNumber() == dst.getNodeNumber()) {
	                return Collections.<LinkEdge>emptyList();
	          }
			 List<List<LinkEdge>> allPath;
			 ImmutablePair<SalNode, SalNode> pair=new ImmutablePair<SalNode, SalNode>(src, dst);
			 int index=pathindex.getOrDefault(pair, 0);
			 if(paths.containsKey(pair)){
				 allPath=paths.get(pair);
			 }
			 else {
				 allPath=new ArrayList<>();
				 Set<SalNode> nodes=new HashSet<>();
				 nodes.add(src);
				 findAllPath(allPath, src, dst,src,nodes,new ArrayList<>());
				 paths.put(pair, allPath);
			}
			
			 if(index==allPath.size()-1){
				 pathindex.put(pair, 0);
			 }
			 else {
				 pathindex.put(pair, index+1);
			}
		...
		private void findAllPath(List<List<LinkEdge>> result,SalNode src, SalNode dst,SalNode curNode,Set<SalNode> nodes,List<LinkEdge> path){
			if(curNode.getNodeNumber()==dst.getNodeNumber()){
				result.add(new ArrayList<>(path));
				return;				
			}
			for(SalNode node:g.getNeighbors(src)){
				if(nodes.contains(node)){
					continue;
				}
				nodes.add(node);
				path.add(findEdge(src, node));
				findAllPath(result, node, dst,node,nodes,path);
