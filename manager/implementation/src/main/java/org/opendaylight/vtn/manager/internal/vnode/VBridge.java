...
import org.opendaylight.vtn.manager.internal.util.vnode.VtnSocket;
...
public final class VBridge extends VirtualBridge<Vbridge> {
	/**
	 * cache save received socket
	 */
	private static LoadingCache<VtnSocket, Boolean> socketCache=CacheBuilder.newBuilder()
			.expireAfterAccess(10, TimeUnit.MINUTES)
			.maximumSize(1000000)
			.build(new CacheLoader<VtnSocket, Boolean>(){
		@Override
		public Boolean load(VtnSocket key) throws Exception {
			// TODO Auto-generated method stub
			return false;
		}});
	/**
	 * a map keeps the port already received
	 */
    ...
    protected void forward(PacketContext pctx, VirtualMapNode vnode)
        ...
        if (ment != null) {
            // Forward the packet.
        	Inet4Packet inet4Packet=pctx.getInet4Packet();
        	if(inet4Packet!=null){
        		L4Packet packet= pctx.getL4Packet();
        		if(packet!=null && packet instanceof PortProtoPacket){
        			PortProtoPacket portProtoPacket=(PortProtoPacket) packet;
        			VtnSocket socket=new VtnSocket();
        			socket.setSrcIp(inet4Packet.getSourceAddress());
        			socket.setDestIp(inet4Packet.getDestinationAddress());
        			socket.setSrcPort(portProtoPacket.getSourcePort());
        			socket.setDestPort(portProtoPacket.getDestinationPort());
        			try {
						if(socketCache.get(socket)){
							return;
						}
        			...
        			forwardtcp(pctx,ment.getPort(),ment.getVlanId(),socket);
        			socketCache.put(socket, true);
        		}
        		else {
