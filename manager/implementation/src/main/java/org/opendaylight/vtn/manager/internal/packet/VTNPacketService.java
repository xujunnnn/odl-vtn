...
public final class VTNPacketService extends SalNotificationListener
    implements PacketfilterListener, VTNInventoryListener {
    /**
     * Logger instance.
     */
	...
	@Override
	public void onAclpacketReceived(AclpacketReceived notification) {
        ...
        PacketReceived packet=new PacketReceivedBuilder().setConnectionCookie(notification.getConnectionCookie())
        												.setFlowCookie(notification.getFlowCookie())
        												.setIngress(notification.getIngress())
        												.setPacketInReason(notification.getPacketInReason())
        												.setPayload(notification.getPayload())
        												.setTableId(notification.getTableId())
        												.build();
        Iterator<VTNPacketListener> it = listeners.iterator();
        if (it.hasNext()) {
            PacketInEvent ev =
                newPacketInEvent(it.next(), packet, ingress);
            if (ev != null) {
                vtnProvider.post(ev);
                while (it.hasNext()) {
